...
class Window_BattleLog < Window_Selectable
    ...
    def current_state_message(enemy)
        return enemy.most_important_state_text if enemy.actor?
        state = enemy.states.each {|state| break state unless state.message3.empty? }
        ...
        case enemy.enemy_id
            ...
            # женский род
            when 285, 24, 25, 107, 192, 193, 234, 235, 237, 238
                case state.id
                    when 83, 112
                        return ' мокрая!'
                end
    ...
    def display_current_state(subject)
      unless subject.most_important_state_text.empty?
        add_text(subject.name + current_state_message(subject))
        wait
      end
    end
    ...
    def added_state_message(enemy, state)
        case enemy.enemy_id
            # средний род
            when 23, 286, 286, 191
                case state.id
                    when 2, 121
                        return ' отравлено!'
                    when 32
                        return ' было вспорото!'
                    when 58
                        return ' покрыто грязью!'
                end
            # женский род
            when 285, 24, 25, 107, 192, 193, 234, 235, 237, 238
                case state.id
                    when 2, 121
                        return ' отравлена!'
                    when 32
                        return ' была вспорота!'
                    when 58
                        return ' покрыта грязью!'
                end
            # мн. ч
            when 202
                case state.id
                    when 2, 121
                        return ' отравлены!'
                    when 32
                        return ' были отравлены!'
                    when 58
                        return ' покрыты грязью!'
                end
        end
    ...
    def display_added_states(target)
      target.result.added_state_objects.each do |state|
        state_msg = target.actor? ? state.message1 : added_state_message(target, state)
